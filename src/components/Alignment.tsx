...
/**
 * AlignmentBar component - Displays a toolbar for toggling X-ray feature.
 * @param {AlignmentTogglerProps} props - React props.
 * @returns {JSX.Element} The rendered AlignmentBar component.
 */
const AlignmentBar = ({ attributes, setAttributes }: AlignmentTogglerProps) => {
  const { alignment } = attributes;

  /**
   * Creates an SVG icon for the alignment feature.
   * @param {string} fillColor - The color to fill the SVG path with.
   * @returns {JSX.Element} The SVG icon.
   */
  const createAlignmentIcon = (fillColor = "none") => (
  ...
  // Define inactive and active alignment icons
  const inactiveAlignmentIcon = createAlignmentIcon("#D3D3D3");
  const activeAlignmentIcon = createAlignmentIcon("#000");

  /**
   * Toggles the alignment attribute.
   * @param {boolean} newAlignment - The new alignment state.
   */
  ...
  return (
    <ToolbarGroup>
      <ToolbarItem>
        {() => (
          <ToolbarDropdownMenu
            icon={(alignment === 'left') ? alignLeft : alignRight}
