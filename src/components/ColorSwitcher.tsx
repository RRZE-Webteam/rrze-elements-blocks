...
type ExtendedColorSwitcherProps = {
  attributes: {
    color: string;
  };
  setAttributes: (newAttributes: { color: string }) => void;
...
const ExtendedColorSwitcher = ({ attributes, setAttributes }: ExtendedColorSwitcherProps) => {
  const { color } = attributes;
  const onChangeColor = (newColor: string) => {
    let colorEntry: any = "";
    if (isUndefined(newColor)) {
      ...
      colorEntry = extendedColorData.find((entry) => entry.color === newColor);
    }
    if (colorEntry) {
      setAttributes({ color: colorEntry.slug });
    } 
  ...
  return (
    <PanelBody title={__("Color Settings", "rrze-elements-b")}>
      <ColorPalette
        ...
        // value={extendedColorData.find((entry) => entry.slug === color).color}
        onChange={onChangeColor}
        disableCustomColors={true}
        asButtons={false}
        clearable={false}
...
const ExtendedColorSwitcherToolbar = ({
  attributes,
  setAttributes,
}: ExtendedColorSwitcherProps) => {
  const { color } = attributes;
  const setColor = (newColor: string) => {
    const colorEntry = extendedColorData.find((entry) => entry.color === newColor);
    if (colorEntry) {
      setAttributes({ color: colorEntry.slug });
    } 
  };
      ...
      <ToolbarItem>
        {() => (
          <ToolbarDropdownMenu
            ...
            className={`rrzeElementsBFakColorSelector ${attributes.color}`}
            label={__("Select a Color", "rrze-elements-b")}
            controls={extendedColorData.map((entry) => ({
              key: entry.slug,
              title: entry.name,
              icon: colorIcon,
              onClick: () => setColor(entry.color),
...
export { ColorSwitcher, ColorSwitcherToolbar, ExtendedColorSwitcher, ExtendedColorSwitcherToolbar };