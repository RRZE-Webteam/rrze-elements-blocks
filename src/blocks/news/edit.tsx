...
export default function Edit({attributes, setAttributes}: EditProps) {
  ...
  const [isFAUElemental, setIsFAUElemental] = useState(false);

  const brickIcon = <SVG xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px"
                         fill="evenodd"><Path
    d="M80-160v-480h120v-160h240v160h80v-160h240v160h120v480H80Zm80-80h640v-320H160v320Zm120-400h80v-80h-80v80Zm320 0h80v-80h-80v80ZM160-240h640-640Zm120-400h80-80Zm320 0h80-80Z"/></SVG>
  const codeIcon = <SVG xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px"
                        fill="evenodd"><Path
    d="M320-240 80-480l240-240 57 57-184 184 183 183-56 56Zm320 0-57-57 184-184-183-183 56-56 240 240-240 240Z"/></SVG>

  useEffect(() => {
    apiFetch({path: '/wp/v2/themes?status=active'}).then(([theme]) => {
      if (theme.name.raw === 'FAU-Elemental') {
        setIsFAUElemental(true);
      }
    });
  }, []);
    ...
    <div {...props}>
      <BlockControls>
        ...
        {isFAUElemental &&
          <>
            <ToolbarDropdownMenu
              icon={attributes.legacyMode ? codeIcon : brickIcon}
              label={__("Mirror Settings in FAU Elemental", "rrze-elements-blocks")}
              controls={[
                {
                  title: attributes.display !== 'list' ? __("Mirror the FAU-Teaser-Grid Block", "rrze-elements-blocks") : __("Disabled as long as you are using the List view", "rrze-elements-blocks"),
                  isDisabled: !attributes.legacyMode || attributes.display === 'list',
                  onClick: () => {
                    setAttributes({
                      legacyMode: false
                    })
                ...
                {
                  title: __("Mirror the legacy Shortcode", "rrze-elements-blocks"),
                  isDisabled: attributes.legacyMode,
                  onClick: () => {
                    setAttributes({
                      legacyMode: true
                    })
      ...
      <InspectorControls>
        {(isFAUElemental && attributes.display !== 'list' && !attributes.legacyMode) ?
          <>
            <PanelBody
