...
test.describe(() => {
    test('Custom News Block is present in the block inserter', async ({ page, admin, editor }) => {
        await admin.visitAdminPage('/');
        await page.goto(process.env.WP_PLAYWRIGHT_TESTPAGE);
        
        await editor.insertBlock({
            name: 'rrze-elements/news'
        })

        await page.waitForTimeout(1000);
        const blockDiv = page.locator('div.wp-block-rrze-elements-news');
