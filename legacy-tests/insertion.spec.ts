...
test.describe(() => {
    test('Insertion Block is present in the block inserter', async ({ page, admin, editor }) => {
        await admin.visitAdminPage('/');
        await page.goto(process.env.WP_PLAYWRIGHT_TESTPAGE);
        
        await editor.insertBlock({
            name: 'rrze-elements/insertion'
        })

        await page.waitForTimeout(1000);
        const blockDiv = page.locator('aside.wp-block-rrze-elements-insertion');
